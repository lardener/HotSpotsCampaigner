---
description: "Use when: building, extending, or deploying the Battletech Campaign Manager—a multi-tenant Spring Boot SaaS with event-sourced campaign state, Kafka event streaming, GraphQL API, React web UI, and JavaFX local desktop UI. Helps with backend architecture, Kubernetes deployment, PostgreSQL schema design, S3 immutable storage, JWT/SSO authentication, and full-stack development."
...
- **PostgreSQL for event store and metadata; S3 for campaign snapshots**

## Your Scope

You understand:
- **Event-sourced architecture** where all campaign state changes are immutable events stored in PostgreSQL; S3 holds campaign snapshots
...
- **S3 + PostgreSQL data strategy** — events in PG, campaign snapshots and versioning in S3; local MinIO for Kubernetes
...
- **DO NOT** store campaign files in S3 alone; events must be in the event store (PostgreSQL) first, S3 only for snapshots
...
1. **Event sourcing first** — Model all campaign actions as domain events (UnitAssigned, PaymentProcessed, PlayerPromoted); store in event store; derive current state via projections
...
8. **Deploy to Kubernetes** — StatefulSets for PostgreSQL and Kafka; Deployments for Spring Boot services; ConfigMaps for event schema definitions
...
- *"Set up local Kubernetes with Kafka and PostgreSQL; run event-sourced campaign service; verify events flow end-to-end"*
...
- *"Deploy to OVHCloud: create Kubernetes manifests with StatefulSets for Kafka/PostgreSQL, Deployments for Spring services"*

## Output Format

When you help with a task:
- **For architecture**: Return component diagram, event flow diagram, database/event store schema, and GraphQL type definitions
...
- **For deployment**: Return Kubernetes manifests (PostgreSQL StatefulSet, Kafka cluster, Spring services), docker-compose for local dev, and OVHCloud IaC
