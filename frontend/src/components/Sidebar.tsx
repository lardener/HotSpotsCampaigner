...
export const Sidebar: React.FC<SidebarProps> = ({
    ...
    return (
        <aside className="sidebar-container" style={{ display: 'grid', gridTemplateRows: 'auto 1fr auto', height: '100vh', width: isCollapsed ? '60px' : '280px', borderRight: '1px solid var(--terminal-border)', transition: 'width 0.3s ease' }}>
            <div className="sidebar-header-main" style={{ padding: isCollapsed ? '10px 5px' : '20px', borderBottom: '1px solid var(--terminal-border)', position: 'relative' }}>
                <button
                    type="button"
                    className="mode-btn theme-amber"
                    style={{ position: 'absolute', right: isCollapsed ? '50%' : '10px', top: '10px', transform: isCollapsed ? 'translateX(50%)' : 'none', padding: '2px 6px', fontSize: '0.8rem', zIndex: 10, minWidth: 'auto' }}
            ...
            <div className="sidebar-content-scroll" style={{ overflowY: 'auto', padding: '10px 0', display: isCollapsed ? 'none' : 'block' }}>
                <NavigationTree data={treeData} onSelect={onSelect} selectedId={selectedId} />
            </div>
            <div className="sidebar-footer-main" style={{ padding: isCollapsed ? '10px 5px' : '20px', borderTop: '1px solid var(--terminal-border)', backgroundColor: 'rgba(0,0,0,0.3)', textAlign: isCollapsed ? 'center' : 'left' }}>
                {!isCollapsed ? (
                    <>
                    ...
                    <span onClick={onToggleCollapse} style={{ cursor: 'pointer' }} title={user?.displayName || user?.name || 'Commander'}>👤</span>
                )}
            </div>
        </aside>
    );
