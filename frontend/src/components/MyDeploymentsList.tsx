...
export const MyDeploymentsList: React.FC<MyDeploymentsListProps> = ({
  commands,
  onSelectDetachment,
}) => {
  const deployedDetachments = commands.flatMap(
    (cmd) =>
      cmd.detachments
        ?.filter((det) => det?.campaignId)
        .map((det) => ({
          ...det!,
  ...
  return (
    <div
      className="container my-deployments-container"
      style={{ position: 'relative', background: 'transparent', minHeight: '100%' }}
    >
      ...
      >
        {deployedDetachments.length === 0 ? (
          <div className="placeholder-content" style={{ border: '1px dashed #444' }}>
            ...
            <p>
              Your mercenary commands currently have no detachments assigned to active campaigns.
            </p>
          </div>
        ) : (
          deployedDetachments.map((det) => (
            <div
              ...
              onClick={() =>
                onSelectDetachment({
                  ...
                  label: `${det.commandName} - ${det.name}`,
                  type: 'DETACHMENT',
                  metadata: {
                    detachmentId: det.id,
                    commandId: det.commandId,
                    campaignId: det.campaignId,
                  },
                })
              }
            ...
            >
              ...
              >
                ...
                <span className="restricted-text" style={{ color: 'var(--terminal-green)' }}>
                  [ DEPLOYED ]
                </span>
              </div>
              <div
              ...
              >
                ...
                <div>
                  ...
                  {det.campaignName || 'UNKNOWN THEATER'}
                </div>
                <div>
                  <span
                    className="restricted-text"
                  ...
                  <span style={{ color: 'var(--terminal-amber)' }}>{det.campaignRating || 0}</span>
                </div>
              </div>
            </div>
          ))
