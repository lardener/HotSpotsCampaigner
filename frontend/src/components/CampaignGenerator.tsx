...
export const CampaignGenerator: React.FC<Props> = ({ user, onSaveSuccess }) => {
  ...
  const {
    ...
    error: metadataError,
    data: metadataData,
  } = useQuery<GetCampaignMetadataQuery>(GetCampaignMetadataDocument)

  useEffect(() => {
    if (metadataData?.publicCampaignMetadata) {
      setPrimaryMissions((metadataData.publicCampaignMetadata.missions?.primary as string[]) ?? [])
      setOpponentMissions(
        (metadataData.publicCampaignMetadata.missions?.opponent as string[]) ?? [],
      )
  ...
  const [createCampaign, { loading: saveLoading }] = useMutation<
    CreateCampaignMutation,
  ...
  const getSaveParams = (): CampaignCreateInput => {
    ...
    const employerCategorySuffix =
      primaryParts.length > 1 ? primaryParts.slice(1).join(': ') : undefined

    const opponentParts = opponent.employerCategory.split(': ')
    const opponentFaction = opponentParts[0]
    const opponentCategorySuffix =
      opponentParts.length > 1 ? opponentParts.slice(1).join(': ') : undefined
  ...
  const updateProposalContract = (
    index: number,
    field: keyof ContractPreview,
    value: string | number,
  ) => {
    if (!proposal) return
    ...
    const updatedCampaign = { ...proposal.campaign }
    if (
      !isNameManuallyEdited &&
      index === 0 &&
      (field === 'missionType' || field === 'employerCategory')
    ) {
      const mission = field === 'missionType' ? String(value) : contractToUpdate.missionType
      const employerStr =
        field === 'employerCategory' ? String(value) : contractToUpdate.employerCategory
    ...
    >
      ...
      {proposal && (
        <div className="proposal-view" style={{ marginTop: '30px' }}>
          <div
            className="campaign-summary-header"
            style={{
              marginBottom: '20px',
              padding: '15px',
          ...
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div>
                {' '}
                {/* Added title to input */}
                <label htmlFor="proposal-system" className="restricted-text sm-text">
                  STAR SYSTEM
                </label>
                <div className="status-bar theme-red cursor-pointer" style={{ padding: '0 5px' }}>
                  <input
                    ...
                    style={{ border: 'none' }}
                    value={proposal.campaign.systemName}
                    onChange={(e) => updateProposalCampaign('systemName', e.target.value)}
                    placeholder="Terra..."
                    title="Star system location"
                  />
                </div>
              </div>
              <div>
                {' '}
                {/* Added title to input */}
                <label htmlFor="proposal-tracks" className="restricted-text sm-text">
                  TRACKS
                </label>
                <div className="status-bar theme-red cursor-pointer" style={{ padding: '0 5px' }}>
                  <input
                    id="proposal-tracks"
                    type="number"
                    value={proposal.campaign.trackCount}
                    onChange={(e) => updateProposalCampaign('trackCount', e.target.value)} // Update state on change
                    onBlur={(e) =>
                      updateProposalCampaign('trackCount', parseInt(e.target.value) || 0)
                    } // Trigger backend call on blur
            ...
            <div className="mt-10 fs-09em">
              {proposal.tracks.map((t: ProposedTrack, idx: number) => (
                <div key={idx} className="mb-5">
                  <label htmlFor={`track-${idx}`} className="restricted-text sm-text">
                    TRACK {idx + 1}
                  </label>{' '}
                  {/* Added title to select */}
                  <div className="status-bar theme-red cursor-pointer" style={{ padding: '0 5px' }}>
                    <select
                      id={`track-${idx}`}
                      value={t.name}
                      onChange={(e) => updateProposalTrack(idx, e.target.value)}
                      className="table-input"
                      style={{ border: 'none' }}
                      title={`Select track type for track ${idx + 1}`}
                    >
                  ...
                  {(t.complication !== 'None' || t.oppositionComplication !== 'None') && (
                    <div
                      className="sm-text mt-5 opacity-08"
                      style={{ color: '#aaa', fontStyle: 'italic' }}
                    >
                      {t.complication !== 'None' && (
                        <div className="mb-2">PRIMARY: {t.complication}</div>
                      )}
                      {t.oppositionComplication !== 'None' && (
                        <div>OPPOSITION: {t.oppositionComplication}</div>
                      )}
                    </div>
                  )}
                </div>
          ...
          <div className="contract-grid">
            {proposal.contracts.map((c, i) => (
              <div
                ...
                style={{ marginBottom: '20px', border: '1px solid #444', padding: '15px' }}
              >
                <div className="restricted-text mb-10">
                  {c.primaryContract ? 'PRIMARY CONTRACT OFFER' : 'OPPOSITION CONTRACT OFFER'}
                </div>
                <p>
                  <label className="restricted-text sm-text block-label">EMPLOYER</label>
                  <div className="status-bar theme-red cursor-pointer" style={{ padding: '0 5px' }}>
                    <input
                      type="text"
                      value={c.employerCategory}
                      onChange={(e) =>
                        updateProposalContract(i, 'employerCategory', e.target.value)
                      }
                      className="table-input w-100"
                ...
                <p>
                  <label className="restricted-text sm-text block-label">MISSION</label>
                  <div className="status-bar theme-red cursor-pointer" style={{ padding: '0 5px' }}>
                    <select
                      className="table-input w-100"
                      style={{ border: 'none' }}
                      value={c.missionType}
                      onChange={(e) => updateProposalContract(i, 'missionType', e.target.value)}
                      title="Mission Type"
                    >
                      {(c.primaryContract ? primaryMissions : opponentMissions).map((m) => (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                  <p>
                    ...
                    <div
                      className="status-bar theme-red cursor-pointer"
                      style={{ padding: '0 5px', display: 'inline-block' }}
                    >
                      <select
                        className="table-input"
                        style={{ border: 'none' }}
                        value={c.payStep}
                        onChange={(e) => updateContractByStep(i, 'pay', parseInt(e.target.value))}
                        title="Pay Step"
                      >
                        {Object.keys(resolvedSteps).map((s) => (
                  ...
                  <p>
                    ...
                    >
                      <select
                        ...
                        value={c.salvageStep}
                        onChange={(e) =>
                          updateContractByStep(i, 'salvage', parseInt(e.target.value))
                        }
                        title="Salvage Step"
                      >
                        {Object.keys(resolvedSteps).map((s) => (
                  ...
                  <p>
                    ...
                    >
                      <select
                        ...
                        value={c.supportStep}
                        onChange={(e) =>
                          updateContractByStep(i, 'support', parseInt(e.target.value))
                        }
                        title="Support Step"
                      >
                        {Object.keys(resolvedSteps).map((s) => (
                  ...
                  <p>
                    ...
                    >
                      <select
                        className="table-input"
                        style={{ border: 'none' }}
                        value={c.transportStep}
                        onChange={(e) =>
                          updateContractByStep(i, 'transport', parseInt(e.target.value))
                        }
                        title="Transport Step"
                      >
                        {Object.keys(resolvedSteps).map((s) => (
                          <option key={s} value={s}>
                            Step {s}
                  ...
                  <p>
                    ...
                    >
                      <select
                        ...
                        value={c.commandStep}
                        onChange={(e) =>
                          updateContractByStep(i, 'command', parseInt(e.target.value))
                        }
                        title="Command Step"
                      >
                        {Object.keys(resolvedSteps).map((s) => (
