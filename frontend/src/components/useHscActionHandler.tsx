...
export const useHscActionHandler = ({
  campaign,
  userCommands,
  setOverlay,
  onActionComplete,
}: UseHscActionHandlerProps) => {
  const [procureAssetData, setProcureAssetData] = useState<ProcureAssetData | null>(null)
  const [procureTargetDetachment, setProcureTargetDetachment] = useState<ExtendedDetachment | null>(
    null,
  )
  ...
  const handleHscAction = useCallback(
    (url: string) => {
      try {
        ...
        const myDetachmentsInCampaign = (campaign.participatingDetachments || [])
          ...
          .map((det) => {
            ...
            return {
              ...det,
              totalSupportPoints: cmd?.totalSupportPoints || 0,
            } as ExtendedDetachment
          })
        ...
        const selectDetachmentAndOpenEditor = (
          assetData: ProcureAssetData | HirePilotData,
          editorType: 'procure' | 'hire',
        ) => {
        ...
        console.error('Invalid HSC action link', e)
      }
    },
