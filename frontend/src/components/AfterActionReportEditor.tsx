...
export type AarAction =
  ...
  | {
      ...
      unitId: string
      patch: Partial<{ status: string; ammo: NumericInput }>
    }
  | {
      type: 'UPDATE_PILOT_STATE'
      pilotId: string
      patch: Partial<{ injuries: number; healed: number }>
    }
  ...
  switch (action.type) {
    case 'SYNC_PROPS': {
      ...
      campaign.participatingDetachments
        ?.filter((det): det is Detachment => det != null)
        .forEach((det) => {
      ...
      return {
        ...
        afterActionNarrative: state.isNarrativeDirty
          ? state.afterActionNarrative
          : track.afterActionNarrative || '',
...
export const AfterActionReportEditor: React.FC<AfterActionReportEditorProps> = ({
  campaign,
  track,
  ...
  const { loading: metadataLoading, data: queryMetaData } = useQuery<GetCampaignMetadataQuery>(
    GetCampaignMetadataDocument,
    {
      skip: !!propMetaData,
    },
  ...
  const {
    ...
    userCommands,
    setOverlay,
    onActionComplete: onLedgerEntryAdded,
  })
  ...
  const handleAwardToDetachment = async (detId: string, cmdId: string | null) => {
    ...
    try {
      ...
      setErrorStates((prev) => ({
        ...prev,
        [noticeKey]: 'COMMUNICATIONS FAILURE: AWARD NOT COMMITTED.',
      }))
  ...
  const handleProcessUnit = (detId: string, cmdId: string | null, unit: CombatUnit) => {
    ...
    const { isTrulyDestroyed } = calculateUnitFinancials(
      unit,
      uState.status,
      repairRules,
      unitStatuses,
  ...
  const commitUnitLogistics = async (detId: string, cmdId: string, unit: CombatUnit) => {
    ...
    const { baseRepairCost, baseReplacementValue, isTrulyDestroyed } = calculateUnitFinancials(
      unit,
      uState.status,
      repairRules,
      unitStatuses,
    ...
    if (isTrulyDestroyed) {
      ...
      const ammoCost =
        state.pricingRule === 'Core'
          ? ammoTons * ammoCostPerTon
          : ammoTons > 0
            ? (campaign.rearmCostPerTonAlphaStrike ?? 20)
            : 0
    ...
    try {
      ...
      if (isTrulyDestroyed) {
        ...
        dispatch({
          type: 'UPDATE_UNIT_STATE',
          unitId: unit.id,
          patch: { status: unitStatuses[0], ammo: 0 },
        })
      }
      ...
      addNotice(
        noticeKey,
        `✓ LOGISTICS COMPLETE: ${finalAmount !== 0 ? Math.abs(finalAmount) + ' SP RECORDED' : 'STATUS RESTORED'}`,
      )
    } catch (err) {
      setErrorStates((prev) => ({
        ...prev,
        [noticeKey]: 'LOGISTICS FAILURE: DATA UPLOAD ABORTED.',
      }))
  ...
  const handleProcessPilot = async (detId: string, cmdId: string | null, pilot: Pilot) => {
    ...
    const isPilotDead =
      state.pricingRule === 'Alpha Strike' ? pState.injuries === 2 : pState.injuries === 6
  ...
  const commitPilotLogistics = async (
    detId: string,
    cmdId: string,
    pilot: Pilot,
    pState: { injuries: number; healed: number },
    terms: { support: SupportTerms },
    noticeKey: string,
    isPilotDead: boolean,
  ) => {
    ...
    try {
      // If pilot is dead, no compensation is due, so skip ledger entry
      ...
      if (isPilotDead) {
        ...
        dispatch({
          type: 'UPDATE_PILOT_STATE',
          pilotId: pilot.id,
          patch: { injuries: newWounds, healed: 0 },
        })
      }
      ...
      setErrorStates((prev) => ({
        ...prev,
        [noticeKey]: 'MEDICAL DATABASE ERROR: RECORD REJECTED.',
      }))
    ...
    >
      ...
      >
        <div className="tactical-panel narrative-editor mb-20 theme-red" data-id="AAR-NARRATIVE">
          <div
            ...
            style={{ borderBottom: '1px solid var(--terminal-amber-dim)', paddingBottom: '5px' }}
          >
            <h3 className="zone-header" style={{ margin: 0 }}>
              ...
              <div style={{ flex: 1 }}>
                {state.afterActionNarrative ? (
                  ...
                  <span className="restricted-text subdued">
                    NO NARRATIVE RECORDED. CLICK EDIT TO INITIALIZE DEBRIEFING.
                  </span>
                )}
              </div>
            </div>
        ...
        {campaign.participatingDetachments
          ...
          .map((det) => (
            <div
              key={det.id}
              className="dashboard-section mb-20"
              style={{
            ...
            >
              <div
                className="flex-between mb-15"
                style={{ borderBottom: '1px solid var(--accent-dim)', paddingBottom: '8px' }}
              >
                <h4 className="terminal-text" style={{ margin: 0 }}>
                  {det.name?.toUpperCase() ?? 'DETACHMENT'}
                </h4>
              ...
              <section className="mb-20">
                ...
                <div className="grid-5-col flex-gap-10" style={{ alignItems: 'center' }}>
                  <div className="flex-col items-center">
                    <label className="xs-text opacity-70" style={{ marginBottom: '4px' }}>
                      CONTRACT
                    </label>
                    <div
                      className="status-bar theme-red cursor-pointer"
                      style={{ padding: '0 5px' }}
                    >
                      <select
                        className="table-input w-100"
                        ...
                        onChange={(e) =>
                          dispatch({
                            type: 'UPDATE_DETACHMENT_AAR',
                            detId: det.id,
                            patch: { selectedContractId: e.target.value },
                          })
                        }
                        ...
                        {campaign.contracts
                          ...
                          .map((c) => (
                            <option key={c.id ?? ''} value={c.id ?? ''}>
                              {c.primaryContract ? 'PRIMARY' : 'OPPOSITION'} (
                              {c.employerCategory ?? 'UNKNOWN'})
                            </option>
                          ))}
                      </select>
                    </div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          dispatch({
                            type: 'UPDATE_DETACHMENT_AAR',
                            detId: det.id,
                            patch: { selectedLevel: parseInt(e.target.value) },
                          })
                        }
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          dispatch({
                            type: 'UPDATE_DETACHMENT_AAR',
                            detId: det.id,
                            patch: { outcomeMultiplier: parseFloat(e.target.value) },
                          })
                        }
                  ...
                  <div className="flex-col items-center">
                    ...
                    <div
                      className={`status-bar theme-red ${isInputInvalid(state.detachmentAars[det.id]?.salvageValue) ? 'invalid' : ''}`}
                      style={{ padding: '0 5px' }}
                    >
                      <input
                        type="number"
                        ...
                        onChange={(e) =>
                          dispatch({
                            type: 'UPDATE_DETACHMENT_AAR',
                            detId: det.id,
                            patch: { salvageValue: e.target.value },
                          })
                        }
                  ...
                  <div className="flex-col items-center">
                    ...
                    <div
                      className={`status-bar theme-red ${isInputInvalid(state.detachmentAars[det.id]?.customAward) ? 'invalid' : ''}`}
                      style={{ padding: '0 5px' }}
                    >
                      <input
                        type="number"
                        ...
                        onChange={(e) =>
                          dispatch({
                            type: 'UPDATE_DETACHMENT_AAR',
                            detId: det.id,
                            patch: { customAward: e.target.value },
                          })
                        }
                ...
                <div className="flex-between mt-10">
                  <div
                    className="status-bar theme-green sm-text flex-grow mr-10"
                  ...
                  >
                    {(() => {
                      ...
                      const tooltip =
                        `Combat Pay: ${financials.baseCombatPay} SP * ${financials.outcomeMultiplier} (Outcome) * ${financials.selectedLevel} (Level) = ${financials.payAward} SP\n` +
                      ...
                      return (
                        <div title={tooltip}>
                          ...
                          <span className="opacity-70 ml-10">
                            (Pay: {financials.payAward} | Support: {terms.support.type}/
                            {terms.support.pct * 100}% | Salvage: {financials.salvageAward} @{' '}
                            {financials.salvageCoverage * 100}%)
                          </span>
                        </div>
                      )
                    })()}
                  ...
                  {notices[`${det.id}-award`] && (
                    <div className="restricted-text theme-green xs-text mr-10 blink-slow">
                      {notices[`${det.id}-award`]}
                    </div>
                  )}
                  {errorStates[`${det.id}-award`] && (
                    <div
                      className="restricted-text xs-text mr-10 blink-slow"
                      style={{ color: 'var(--terminal-alert)' }}
                    >
                      {errorStates[`${det.id}-award`]}
                    </div>
                  )}
              ...
              <div className="grid-2-col" style={{ gap: '20px' }}>
                <div>
                  ...
                  <table className="tactical-table sm-text">
                    ...
                    <tbody>
                      {[...(det.units || [])]
                        ...
                        .map((u) => {
                          const uState = state.unitStates[u.id] || {
                            status: u.status || unitStatuses[0],
                            ammo: 0,
                          }
                          const ammoTons = parseNumericInput(uState.ammo)
                          ...
                          if (isTrulyDestroyed) {
                            if (terms.support.type === 'BATTLE') {
                              displayAmount =
                                -1 * Math.ceil(baseReplacementValue * terms.support.pct)
                              tooltip =
                                `Replacement Value: ${financials.bv} BV * 0.5${techTax !== 1 ? ` * ${techTax} (Tech)` : ''} = ${baseReplacementValue} SP\n` +
                                `Battle Support: ${terms.support.pct * 100}% = +${displayAmount} SP`
                            } else {
                              displayAmount = 0
                              tooltip =
                                `Replacement Value: ${baseReplacementValue} SP\n` +
                            ...
                            tooltip =
                              `Base Repair: ${financials.costBasis}${state.pricingRule === 'Core' ? 'T' : ' Basis'} * ${financials.damageMultiplier} (Status)${financials.unitModifier !== 1 ? ` * ${financials.unitModifier} (Type)` : ''}${techTax !== 1 ? ` * ${techTax} (Tech)` : ''} = ${repairCost} SP\n` +
                              `${rearmPart}\n` +
                            ...
                            <tr key={u.id}>
                                ...
                                >
                                  <select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      updateUnit({
                                        variables: { id: u.id, input: { status: newStatus } },
                                      })
                                    }}
                                    title="Select unit status"
                                  >
                                    {unitStatuses.map((s: string) => (
                              ...
                              <td className="text-center">
                                {isTrulyDestroyed ? (
                                  ...
                                  <div
                                    className={`status-bar theme-red ${isInputInvalid(uState.ammo) ? 'invalid' : ''}`}
                                    style={{ padding: '0 5px', width: '50px', margin: '0 auto' }}
                                  >
                                    <input
                                      type="number"
                              ...
                              <td className="text-right" title={tooltip}>
                              ...
                              <td className="text-right">
                                <button
                                  ...
                                  onClick={() =>
                                    handleProcessUnit(det.id, det.mercenaryCommandId ?? null, u)
                                  }
                                  disabled={
                                    loadingStates[unitNoticeKey] ||
                                    isInputInvalid(uState.ammo) ||
                                    uState.ammo === '-'
                                  }
                  ...
                  {[...(det.units || [])]
                    ...
                    .map((u) => {
                      const isTrulyDestroyed =
                        (state.unitStates[u.id]?.status || u.status) === unitStatuses[5]
                      return (
                        notices[`${u.id}-logistics`] &&
                        !isTrulyDestroyed && (
                          <div
                            key={`notice-${u.id}`}
                            className="restricted-text theme-green xs-text mt-5 text-center"
                          >
                            {u.model}: {notices[`${u.id}-logistics`]}
                          </div>
                        )
                      )
                    })}
                  {[...(det.units || [])]
                    ...
                    .map((u) => {
                      const isTrulyDestroyed =
                        (state.unitStates[u.id]?.status || u.status) === unitStatuses[5]
                      return (
                        errorStates[`${u.id}-logistics`] &&
                        !isTrulyDestroyed && (
                          <div
                            key={`error-${u.id}`}
                            className="restricted-text xs-text mt-5 text-center blink-slow"
                            style={{ color: 'var(--terminal-alert)' }}
                          >
                            {u.model}: {errorStates[`${u.id}-logistics`]}
                          </div>
                        )
                      )
                    })}
                ...
                <div>
                  ...
                  <table className="tactical-table sm-text">
                    ...
                    <tbody>
                      {' '}
                      {/* Use pilot.id for key */}
                      {[...(det.pilots || [])]
                        .filter((p): p is Pilot => p != null)
                        .sort((a, b) => (a.name ?? '').localeCompare(b.name ?? ''))
                        .map((p) => {
                          const pState = state.pilotStates[p.id] || {
                            injuries: p.wounds || 0,
                            healed: 0,
                          }
                          const terms = getDetachmentTerms(det.id)
                          const financials = calculatePilotFinancials(
                            pState,
                            terms,
                            injuryHealCost,
                            state.pricingRule,
                          ...
                          const tooltip =
                            `Medical Cost: ${financials.healed} injuries x ${financials.injuryHealCost} SP = ${financials.rawMedicalCost} SP\n` +
                          ...
                          return (
                            <tr key={p.id}>
                                ...
                                >
                                  <select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      updatePilot({
                                        variables: { id: p.id, input: { wounds: val } },
                                      })
                                    }}
                                    title={
                                      state.pricingRule === 'Alpha Strike'
                                        ? 'Select pilot status'
                                        : 'Select total pilot injuries'
                                    }
                                  >
                                    {state.pricingRule === 'Alpha Strike'
                                      ? [0, 1, 2].map((v) => (
                                          <option key={v} value={v}>
                                            {v === 2 ? 'FATAL' : v === 1 ? 'WOUNDED' : 'ACTIVE'}
                                          </option>
                                        ))
                                      : [0, 1, 2, 3, 4, 5, 6].map((v) => (
                                          <option key={v} value={v}>
                                  ...
                                  <select
                                    ...
                                    onChange={(e) => {
                                      dispatch({
                                        type: 'UPDATE_PILOT_STATE',
                                        pilotId: p.id,
                                        patch: { healed: parseInt(e.target.value) },
                                      })
                                    }}
                              ...
                              <td className="text-right" title={tooltip}>
                                {pilotDisplayCost > 0 ? `+${pilotDisplayCost}` : pilotDisplayCost}
                              </td>
                              <td className="text-right">
                                <button
                                  className="mode-btn sm-text"
                                  onClick={() =>
                                    handleProcessPilot(det.id, det.mercenaryCommandId ?? null, p)
                                  } // The actual ledger entry amount is calculated inside handleProcessPilot
      ...
      {showProcureEditor && procureAssetData && procureTargetDetachment && (
        <CombatUnitEditor
          ...
          unit={
            {
              ...
              status:
                (metaData?.publicCampaignMetadata?.unitStatuses as UnitStatus[])?.[0] ||
                'OPERATIONAL',
              detachmentId: procureTargetDetachment.id,
            } as CombatUnit
          }
          unitTypes={
            (metaData?.publicCampaignMetadata?.unitTypes as UnitType[]) || [
              'BM',
              'CV',
              'PM',
              'IM',
          ...
          unitStatuses={
            (metaData?.publicCampaignMetadata?.unitStatuses as UnitStatus[]) || FALLBACK_STATUSES
          }
          techBases={
            (metaData?.publicCampaignMetadata?.techBases as TechBase[]) || [
              'Inner Sphere',
              'Clan',
              'Mixed',
            ]
