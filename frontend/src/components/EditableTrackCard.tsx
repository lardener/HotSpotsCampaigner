...
export const EditableTrackCard: React.FC<EditableTrackCardProps> = ({
  ...
  const [oppositionComplications, setOppositionComplications] = useState(
    track.oppositionComplications ?? '',
  )
  const [location, setLocation] = useState(track.location ?? '')
  const [nextSession, setNextSession] = useState(
    track.nextSession ? track.nextSession.substring(0, 16) : '',
  )
  ...
  return (
    ...
    >
      <div className="flex-between mb-5" style={{ alignItems: 'flex-start' }}>
        ...
        >
          ...
          <datalist id={`track-types-${track.id}`}>
            {metaData?.publicCampaignMetadata?.trackTypes
      ...
      <div className="flex flex-gap-5 mb-5">
        <div style={{ flex: 1 }}>
          <div
            className="restricted-text mb-2"
            style={{ fontSize: '0.55rem', opacity: 0.8, color: 'var(--terminal-red)' }}
          >
      ...
      <div className="flex flex-gap-5 mb-5">
        <div
          className="status-bar theme-amber"
          style={{ flex: 1, padding: '0 5px', display: 'flex', alignItems: 'center' }}
        >
          <input
            className="table-input"
      ...
      <div className="flex-between">
        <div
          className="status-bar theme-amber"
          style={{ width: '60%', padding: '0 5px', display: 'flex', alignItems: 'center' }}
        >
          <input
            type="datetime-local"
            className="table-input"
        ...
        <div
          className="status-bar theme-amber"
          style={{ width: '35%', padding: '0 5px', display: 'flex', alignItems: 'center' }}
        >
          <select
            className="table-input"
            style={{ fontSize: '0.7rem', width: '100%', border: 'none' }}
          ...
          >
            ...
            {campaign?.factions
              ?.filter((f): f is NonNullable<typeof f> => f != null && f.factionName != null)
              .map((f) => {
                const validContracts = campaign?.contracts?.filter(
                  (c): c is NonNullable<typeof c> => c != null,
                )
                const employerCategory = validContracts?.find(
                  (c) => c.primaryContract && c.employerCategory != null,
                )?.employerCategory
                const isPri =
                  employerCategory != null && employerCategory.startsWith(f.factionName!)
                return (
                  <option key={f.id} value={f.id}>
                    {isPri ? 'Pri: ' : 'Opp: '}
                    {f.factionName!.substring(0, 3).toUpperCase()}
                  </option>
                )
              })}
          </select>
