...
const DroppableZone: React.FC<{ id: string; title: string; children: React.ReactNode }> = ({
  id,
  title,
  children,
}) => {
...
export const ForceDashboard: React.FC<{ commandId: string; initialMode?: ViewMode }> = ({
  commandId,
  ...
  const sortUnits = (list: CombatUnit[]) =>
    [...list].sort(
      (a, b) =>
  ...
  const sortPilots = (list: Pilot[]) =>
    [...list].sort((a, b) => (a.name || '').localeCompare(b.name || ''))

  useEffect(() => {
    if (data?.getCommand) {
      const rawUnits = (data.getCommand.units?.filter(
        (u): u is NonNullable<typeof u> => u != null,
      ) || []) as CombatUnit[]
      const rawPilots =
        data.getCommand.pilots?.filter((p): p is NonNullable<typeof p> => p != null) || []
      const rawDetachments =
        data.getCommand.detachments?.filter((d): d is NonNullable<typeof d> => d != null) || []
  ...
  const unitStatuses = (data?.publicCampaignMetadata?.unitStatuses?.filter(
    (s): s is string => s != null,
  ) || FALLBACK_STATUSES) as UnitStatus[]
  const unitTypes = (data?.publicCampaignMetadata?.unitTypes?.filter(
    (t): t is string => t != null,
  ) || FALLBACK_TYPES) as UnitType[]
  const techBases = (data?.publicCampaignMetadata?.techBases?.filter(
    (t): t is string => t != null,
  ) || FALLBACK_TECH) as TechBase[]
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      if (type === 'UNIT') {
        setUnits((prev) =>
          sortUnits(
            prev.map((u) => (u.id === assetId ? { ...u, detachmentId: targetDetachmentId } : u)),
          ),
        )
      } else {
        setPilots((prev) =>
          sortPilots(
            prev.map((p) => (p.id === assetId ? { ...p, detachmentId: targetDetachmentId } : p)),
          ),
        )
      }
    } catch {
      ...
      <div className="force-dashboard-layout">
        <header className="dashboard-header">
          ...
          <div className="status-bar theme-amber">
            LINK: ACTIVE | REPUTATION: {data?.getCommand?.reputation} | WARCHEST:{' '}
            {data?.getCommand?.totalSupportPoints} SP
          </div>
              ...
              <div className="asset-group">
                <div className="flex-between">
                  ...
                  <button
                    ...
                    title="Procure unit for hangar pool"
                  >
                    +
                  </button>
                </div>
                {units
                  .filter((u) => !u.detachmentId)
                  .map((u) => (
                    <DraggableAsset
                      key={u.id}
                      id={u.id}
              ...
              <div className="asset-group">
                <div className="flex-between">
                  ...
                  <button
                    type="button"
                    className="mode-btn theme-green sm-text"
                    onClick={() => handleHirePilot(null)}
                    title="Hire pilot for barracks"
                  >
                    +
                  </button>
                </div>
                {pilots
                  .filter((p) => !p.detachmentId)
                  .map((p) => (
                    <DraggableAsset
                      key={p.id}
                      id={p.id}
            ...
            <div className="detachments-container">
              {detachments.map((det) => (
                <DroppableZone
                  key={det.id}
                  id={det.id}
                  title={`DETACHMENT: ${det.name}${det.campaignRating != null ? ` (RATING: ${det.campaignRating})` : ''}`}
                >
                  <div className="flex flex-gap-5 justify-end" style={{ marginBottom: '8px' }}>
                    <button
                      type="button"
                      className="mode-btn theme-green xs-text"
                      onClick={() => handleAddUnit(det.id)}
                      title="Add unit directly to this detachment"
                    >
                      +
                    </button>
                    <button
                      type="button"
                      className="mode-btn theme-green xs-text"
                      onClick={() => handleHirePilot(det.id)}
                      title="Add pilot directly to this detachment"
                    >
                      👤+
                    </button>
                  </div>
                  <div className="assigned-assets">
                    {units
                      .filter((u) => u.detachmentId === det.id)
                      .map((u) => (
                        <DraggableAsset
                          key={u.id}
                          id={u.id}
                    ...
                    {pilots
                      .filter((p) => p.detachmentId === det.id)
                      .map((p) => (
                        <DraggableAsset
                          key={p.id}
                          id={p.id}
                ...
                {managedCampaigns.map((camp) => (
                  <div key={camp.id} className="ops-item tactical-panel mb-05rem">
                    ...
                    <div
                      ...
                      title={`System: ${camp.systemName} | Tracks: ${camp.trackCount}`}
                    >
                      SYSTEM: {camp.systemName} | TRACKS: {camp.trackCount}
                    </div>
                    {camp.participatingDetachments && camp.participatingDetachments.length > 0 && (
                      <div
                        className="mt-10"
                        style={{ borderTop: '1px dashed var(--accent-dim)', paddingTop: '5px' }}
                      >
                    ...
                    <button type="button" className="mode-btn theme-blue sm-text mt-10">
                      VIEW THEATER
                    </button>
                  </div>
                ))}
            ...
            <div className="tactical-panel" data-id="PART-01">
              ...
              <div className="ops-list">
                {participatingCampaigns.map((camp) => (
                  <div key={camp.id} className="ops-item tactical-panel mb-05rem">
                    <div className="ops-title">{camp.name}</div>
                    <div className="ops-meta" title={`Employer: ${camp.primaryEmployer}`}>
                      EMPLOYER: {camp.primaryEmployer}
                    </div>
                    {detachments.find((d) => d.campaignId === camp.id)?.campaignRating != null && (
                      <div className="ops-meta">
                        THEATER RATING:{' '}
                        {detachments.find((d) => d.campaignId === camp.id)?.campaignRating}
                      </div>
                    )}
                    {camp.participatingDetachments && camp.participatingDetachments.length > 0 && (
                      <div
                    ...
                    <div className="ops-actions flex flex-gap-10">
                      <button type="button" className="mode-btn theme-green sm-text mt-05rem">
                        OPEN LOGBOOK
                      </button>
                    </div>
                  </div>
