...
const useTheaterCampaignSync = (
  ...
  const campaign: TheaterCampaign = useMemo(() => {
    ...
    const queryCampaign =
      campaignQueryData?.getCampaign?.id === selectedCampaignId
        ? (campaignQueryData.getCampaign as Partial<TheaterCampaign>)
        : {}

    const localIsParticipant =
      userCommands?.some((cmd) =>
        (cmd.detachments || [])
          .filter((d): d is Detachment => d != null)
          .some((det: Detachment) => det.campaignId === selectedCampaignId),
      ) || false
  ...
  const displayMonthCount = useMemo(() => {
    const trackMax = (campaign.tracks || [])
      .filter((t): t is CampaignTrack => t != null)
      .reduce((max: number, t: CampaignTrack) => Math.max(max, t.monthIndex || 1), 0)
  ...
  const opposition = useMemo(
    () =>
      (campaign?.contracts || [])
        ...
        .find((c: Contract) => !c.primaryContract),
    [campaign],
  )

  const campaignInvites = (campaignQueryData?.getCampaign?.campaignInvites || []).filter(
    (i): i is any => i != null,
  ) as any[]
  ...
  useEffect(() => {
    ...
    setArmorMult(
      campaign.armorMultiplier ?? (metaData?.publicCampaignMetadata as any)?.armorMultiplier ?? 0.5,
    )
    setInternalMult(
      campaign.internalMultiplier ??
        (metaData?.publicCampaignMetadata as any)?.internalMultiplier ??
        2.0,
    )
    setCrippledMult(
      campaign.crippledMultiplier ??
        (metaData?.publicCampaignMetadata as any)?.crippledMultiplier ??
        3.0,
    )
    setDestroyedMult(
      campaign.destroyedMultiplier ??
        (metaData?.publicCampaignMetadata as any)?.destroyedMultiplier ??
        5.0,
    )
    setNonMechMod(
      campaign.nonMechModifier ?? (metaData?.publicCampaignMetadata as any)?.nonMechModifier ?? 0.5,
    )
    setMixedTechTax(
      campaign.mixedTechModifier ??
        (metaData?.publicCampaignMetadata as any)?.mixedTechModifier ??
        1.5,
    )
    setClanTechTax(
      campaign.clanTechModifier ??
        (metaData?.publicCampaignMetadata as any)?.clanTechModifier ??
        2.0,
    )

    setOmnimechMod(
      campaign.omnimechReconfigureModifier ??
        (metaData?.publicCampaignMetadata as any)?.omnimechReconfigureModifier ??
        0.5,
    )
    setPurchaseMult(
      campaign.pvPurchaseUnitMultiplier ??
        (metaData?.publicCampaignMetadata as any)?.pvPurchaseUnitMultiplier ??
        40,
    )
    setSellMult(
      campaign.pvSellUnitMultiplier ??
        (metaData?.publicCampaignMetadata as any)?.pvSellUnitMultiplier ??
        20,
    )
    setRearmTon(
      campaign.rearmCostPerTon ?? (metaData?.publicCampaignMetadata as any)?.rearmCostPerTon ?? 10,
    )
    setRearmAS(
      campaign.rearmCostPerTonAlphaStrike ??
        (metaData?.publicCampaignMetadata as any)?.rearmCostPerTonAlphaStrike ??
        20,
    )
    setHireMW(
      campaign.hireMechWarriorCost ??
        (metaData?.publicCampaignMetadata as any)?.hireMechWarriorCost ??
        100,
    )
    setHirePilot(
      campaign.hireNamedPilotCost ??
        (metaData?.publicCampaignMetadata as any)?.hireNamedPilotCost ??
        150,
    )
    setHireBA(
      campaign.hireBattleArmorCost ??
        (metaData?.publicCampaignMetadata as any)?.hireBattleArmorCost ??
        20,
    )
    setHealWound(
      campaign.healMechWarriorPerWoundBoxCost ??
        (metaData?.publicCampaignMetadata as any)?.healMechWarriorPerWoundBoxCost ??
        30,
    )
    setHealMonth(
      campaign.healMechWarriorPerMonthLimit ??
        (metaData?.publicCampaignMetadata as any)?.healMechWarriorPerMonthLimit ??
        2,
    )
    setHealBA(
      campaign.healBattleArmorCost ??
        (metaData?.publicCampaignMetadata as any)?.healBattleArmorCost ??
        10,
    )
    setTrainCmd(
      campaign.trainFormationCommanderCost ??
        (metaData?.publicCampaignMetadata as any)?.trainFormationCommanderCost ??
        500,
    )
    setTrainForm(
      campaign.changeFormationTrainingCost ??
        (metaData?.publicCampaignMetadata as any)?.changeFormationTrainingCost ??
        250,
    )
    setAbility1(
      campaign.learnCommandAbility1Cost ??
        (metaData?.publicCampaignMetadata as any)?.learnCommandAbility1Cost ??
        250,
    )
    setAbility2(
      campaign.learnCommandAbility2Cost ??
        (metaData?.publicCampaignMetadata as any)?.learnCommandAbility2Cost ??
        500,
    )
    setAbility3(
      campaign.learnCommandAbility3Cost ??
        (metaData?.publicCampaignMetadata as any)?.learnCommandAbility3Cost ??
        750,
    )
    setReplaceAbility(
      campaign.replaceCommandAbilityCost ??
  ...
  return {
    ...
    params: {
      ...
      setCampaignLengthInMonths,
      campaignTrackCount,
      setCampaignTrackCount,
    },
    handleUpdate,
...
export const CampaignTheaterView: React.FC<CampaignTheaterViewProps> = ({
  ...
  const {
    loading,
    data: campaignQueryData,
    refetch: refetchCampaign,
  } = useQuery<GetCampaignDetailsQuery>(GET_CAMPAIGN_DETAILS, {
  ...
  const handleDrop = async (e: React.DragEvent, targetMonth: number, targetTrackId?: string) => {
    ...
    try {
      await updateTrack({
        variables: {
          id: draggedTrackId,
          input: { monthIndex: targetMonth } as unknown as TrackUpdateInput,
        },
      })
  ...
  return (
    ...
    >
      ...
      {!selectedCampaignId ? (
          ...
          >
            ...
            >
              {campaignFilter === 'ACTIVE'
                ? '[ VIEWING: ACTIVE ONLY ]'
                : '[ VIEWING: ALL THEATERS ]'}
            </button>
          </header>

          <div
            className="command-panels-list"
          ...
          >
              ...
              .map((camp) => (
                <div
                  ...
                  style={{
                    ...
                    backgroundColor:
                      camp.status === 'ACTIVE' ? 'rgba(51, 255, 51, 0.02)' : 'transparent',
                    position: 'relative',
                  }}
                >
                  <div
                  ...
                  >
                    <h3
                      ...
                      style={{ margin: 0, color: 'var(--terminal-green)' }}
                    >
                      {camp.name}
                    </h3>
                    <span
                      ...
                      style={{
                        color:
                          camp.status === 'ACTIVE' ? 'var(--terminal-amber)' : 'var(--accent-dim)',
                      }}
                    >
                      [ STATUS: {camp.status} ]
                    </span>
                  ...
                  <div
                    style={{
                      display: 'grid',
                      gridTemplateColumns: '1.5fr 1.5fr 1fr 1fr 1.5fr 1fr',
                      gap: '20px',
                      marginTop: '15px',
                    }}
                  >
                    ...
                    <div>
                      ...
                      {camp.secondaryEmployer || 'UNKNOWN'}
                    </div>
                    <div>
                      <span
                        className="restricted-text"
                    ...
                    <div>
                      ...
                      <div style={{ maxHeight: '60px', overflowY: 'auto' }}>
                        {(camp.participatingDetachments || [])
                          .filter((d: Detachment | null | undefined): d is Detachment => d != null)
                          .map((d: Detachment) => (
                            <div
                              key={d.id}
                              className="sm-text"
                        ...
                        {(camp.participatingDetachments || []).filter(
                          (d: Detachment | null | undefined): d is Detachment => d != null,
                        ).length === 0 && <span className="sm-text subdued">NONE</span>}
                      </div>
                    </div>
            ...
            {!loadingManaged && managedData?.managedCampaigns.length === 0 && (
              <div className="placeholder-content" style={{ border: '1px dashed #444' }}>
                ...
                <button className="mode-btn theme-red" onClick={onCreateNew}>
                  START NEW CAMPAIGN
                </button>
              </div>
            )}
        ...
        (() => {
          if (loading && !campaignQueryData) {
            return (
              <div
                className="loading-intel pulse"
                style={{ padding: '100px', textAlign: 'center' }}
              >
                RETRIEVING THEATER DATA...
              </div>
            )
          }
            ...
            return (
              <div className="placeholder-content">
                ...
                <p className="restricted-text">
                  UNABLE TO RETRIEVE OPERATIONAL RECORD FOR THEATER: {selectedCampaignId}
                </p>
                <button className="mode-btn" onClick={onReturnToList}>
                  RETURN TO LIST
                </button>
              </div>
            )
            ...
            <>
              <header className="dashboard-header">
                <div className="flex-between">
                  <div>
                    ...
                    <p className="restricted-text">
                      THEATER COMMAND DATA: {campaign?.systemName?.toUpperCase()}{' '}
                      {isSyncing && <span className="pulse">...SYNCHRONIZING</span>}
                    </p>
                  </div>
                  <div className="flex flex-gap-10">
                    ...
                    {!campaign.isManager && campaign.isParticipant && (
                      <div
                        className="status-bar theme-blue"
                        style={{
                          padding: '0 15px',
                          height: '32px',
                          display: 'flex',
                    ...
                    {!campaign.isManager && !campaign.isParticipant && (
                      ...
                      >
                        <span className="restricted-text" style={{ fontSize: '0.7rem' }}>
                          [ THEATER STATUS: PUBLIC INTEL ]
                        </span>
                      </div>
                    )}
                    <button type="button" className="mode-btn" onClick={onReturnToList}>
                      [ RETURN TO LIST ]
                    </button>
                  </div>
                </div>
              ...
              <div className="mb-30">
                <div className="tactical-panel">
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          {campaign?.description ? (
                            ...
                            <span className="restricted-text subdued">
                              NO OPERATIONAL BRIEFING FILED. CLICK EDIT TO INITIALIZE THEATER LORE.
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  >
                    <h4
                      className="restricted-text mb-10"
                      style={{ fontSize: '0.7rem', color: 'var(--terminal-amber)' }}
                    >
                      CAMPAIGN DURATION
                    </h4>
                    <div
                    ...
                    >
                      <div>
                        <label
                          htmlFor="campaign-length-in-months"
                          className="restricted-text"
                        ...
                        <div
                          className="status-bar theme-amber cursor-pointer"
                          style={{
                            display: 'flex',
                            margin: 0,
                            padding: '0 5px',
                        ...
                        >
                          <input
                            id="campaign-length-in-months"
                            type="number"
                            min="1"
                            className="table-input"
                            style={{ width: '100%', textAlign: 'center', border: 'none' }}
                            value={campaignLengthInMonths}
                            disabled={!campaign.isManager}
                            onChange={(e) =>
                              setCampaignLengthInMonths(Math.max(1, parseInt(e.target.value) || 1))
                            }
                            onBlur={(e) =>
                              handleUpdate(
                                'lengthInMonths',
                      ...
                      <div>
                        ...
                        >
                          <input
                            ...
                            disabled={!campaign.isManager}
                            onChange={(e) =>
                              setCampaignTrackCount(Math.max(1, parseInt(e.target.value) || 1))
                            }
                            onBlur={(e) =>
                              handleUpdate('trackCount', Math.max(1, parseInt(e.target.value) || 1))
                            }
                  ...
                  <details className="mt-20">
                    <summary
                      className="restricted-text cursor-pointer"
                      style={{ fontSize: '0.7rem', color: 'var(--terminal-amber)' }}
                    >
                      [ ADVANCED THEATER SETTINGS & LOGISTICS ]
                    </summary>
                    <div
                      className="tactical-panel mt-10"
                      style={{ padding: '15px', backgroundColor: 'rgba(0,0,0,0.15)' }}
                    >
                      <h5
                        className="restricted-text mb-10"
                        style={{ fontSize: '0.6rem', color: 'var(--terminal-amber)' }}
                      >
                        FINANCIAL PARAMETERS
                      </h5>
                      <div
                        className="grid-4-col mb-20"
                        style={{
                          display: 'grid',
                      ...
                      >
                        <div>
                          ...
                          >
                            <input
                              ...
                              value={monthlyPay}
                              onChange={(e) =>
                                setMonthlyPay(Math.max(0, parseInt(e.target.value) || 0))
                              }
                        ...
                        <div>
                          <label
                            htmlFor="campaign-monthly-maintenance"
                            className="restricted-text"
                            style={{ fontSize: '0.6rem', color: 'var(--terminal-amber)' }}
                          >
                            MAINTENANCE
                          ...
                          >
                            <input
                              id="campaign-monthly-maintenance"
                              type="number"
                              min="0"
                              className="table-input"
                              style={{ width: '100%', textAlign: 'center', border: 'none' }}
                              ...
                              onChange={(e) =>
                                setMonthlyMaintenance(Math.max(0, parseInt(e.target.value) || 0))
                              }
                        ...
                        <div>
                          <label
                            htmlFor="campaign-transportation-cost"
                            className="restricted-text"
                            style={{ fontSize: '0.6rem', color: 'var(--terminal-amber)' }}
                          >
                            TRANS COST
                          ...
                          >
                            <input
                              id="campaign-transportation-cost"
                              type="number"
                              min="0"
                              className="table-input"
                              style={{ width: '100%', textAlign: 'center', border: 'none' }}
                              ...
                              onChange={(e) =>
                                setTransportationCost(Math.max(0, parseInt(e.target.value) || 0))
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              value={combatPay}
                              onChange={(e) =>
                                setCombatPay(Math.max(0, parseInt(e.target.value) || 0))
                              }
                      ...
                      <div
                        className="grid-7-col flex-gap-10 mb-20"
                        style={{
                          display: 'grid',
                      ...
                      >
                        <div>
                          <label
                            ...
                            style={{ fontSize: '0.55rem', color: 'var(--terminal-amber)' }}
                          >
                            REARM TON
                          </label>
                          <div
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('rearmCostPerTon', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('hireMechWarriorCost', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('hireNamedPilotCost', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('hireBattleArmorCost', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('healBattleArmorCost', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('learnCommandAbility1Cost', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('learnCommandAbility2Cost', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('learnCommandAbility3Cost', e.target.value)
                              }
                      ...
                      <h5
                        ...
                        style={{ fontSize: '0.6rem', color: 'var(--terminal-amber)' }}
                      >
                        LOGISTICS & REPAIR MULTIPLIERS
                      </h5>
                      <div
                        className="grid-5-col flex-gap-10"
                        style={{
                          display: 'grid',
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('pvPurchaseUnitMultiplier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleActivityCostUpdate('pvSellUnitMultiplier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleRepairRuleUpdate('armorMultiplier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleRepairRuleUpdate('internalMultiplier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleRepairRuleUpdate('crippledMultiplier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleRepairRuleUpdate('destroyedMultiplier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleRepairRuleUpdate('nonMechModifier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleRepairRuleUpdate('mixedTechModifier', e.target.value)
                              }
                        ...
                        <div>
                          ...
                          >
                            <input
                              ...
                              onBlur={(e) =>
                                handleRepairRuleUpdate('clanTechModifier', e.target.value)
                              }
                  ...
                  <div className="mt-15">
                    <label className="restricted-text" style={{ color: 'var(--terminal-green)' }}>
                      PRIMARY CONTRACT: {campaign?.primaryEmployer}
                    </label>
                    <div className="grid-5-col mt-5">
                      <div>
                        <label className="restricted-text" style={{ fontSize: '0.6rem' }}>
                          PAY (STEP {campaign?.payStep})
                        </label>
                        <div
                          className="status-bar theme-green"
                          style={{
                            display: 'block',
                            margin: 0,
                            fontSize: '0.7rem',
                  ...
                  {opposition && (
                    ...
                    >
                      <label className="restricted-text" style={{ color: 'var(--terminal-red)' }}>
                        OPPOSITION CONTRACT: {opposition.employerCategory}
                      </label>
                      <div
                        style={{
                          display: 'grid',
                        ...
                        <div>
                          ...
                          <div
                            ...
                            style={{
                              ...
                              borderColor: 'var(--terminal-red-dim)',
                            }}
                          >
                            {Math.round((opposition.payRate || 0) * 100)}%
                          </div>
                        </div>
                        <div>
                          <label className="restricted-text" style={{ fontSize: '0.6rem' }}>
                            SALVAGE ({opposition.salvageStep})
                          </label>
                          <div
                            className="status-bar"
                            style={{
                          ...
                          >
                            {opposition.salvageTerms}
                          </div>
                        </div>
                        <div>
                          <label className="restricted-text" style={{ fontSize: '0.6rem' }}>
                            SUPPORT ({opposition.supportStep})
                          </label>
                          <div
                            className="status-bar"
                            style={{
                          ...
                          >
                            {opposition.supportTerms}
                          </div>
                        </div>
                        <div>
                          <label className="restricted-text" style={{ fontSize: '0.6rem' }}>
                            TRANS ({opposition.transportStep})
                          </label>
                          <div
                            className="status-bar"
                            style={{
                          ...
                          >
                            {opposition.transportTerms}
                          </div>
                        </div>
                        <div>
                          <label className="restricted-text" style={{ fontSize: '0.6rem' }}>
                            CMD ({opposition.commandStep})
                          </label>
                          <div
                            className="status-bar"
                            style={{
              ...
              <div className="dashboard-section tactical-panel mb-30">
                ...
                <div
                  className="month-panel-grid mt-15"
                  style={{
                    display: 'grid',
                ...
                >
                  {Array.from({ length: displayMonthCount }, (_, i) => i + 1).map((mIdx) => {
                    const monthTracks = (campaign?.tracks || [])
                      .filter((t): t is CampaignTrack => t != null)
                      .filter((t: CampaignTrack) => (t.monthIndex || 1) === mIdx)
                      .sort(
                        (a: CampaignTrack, b: CampaignTrack) =>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className="track-container flex flex-column flex-gap-10"
                        ...
                        >
                          ...
                          {monthTracks.length === 0 && (
                            <div
                              className="restricted-text subdued"
                              style={{ textAlign: 'center', padding: '20px', fontSize: '0.7rem' }}
                            >
                              NO OPS SCHEDULED
                            </div>
                          )}
              ...
              <div className="dashboard-section tactical-panel">
                ...
                >
                      ...
                      <div key={det.id} className="asset-card" style={{ position: 'relative' }}>
                        ...
                        >
                          ...
                          >
                            ...
                            {det.campaignRating != null && (
                              <span style={{ color: 'var(--terminal-amber)', fontSize: '0.8rem' }}>
                                RATING: {det.campaignRating}
                              </span>
                            )}
                          </div>
                        ...
                        {campaign.isManager && (
                          <button
                            ...
                            onClick={() => handleRemoveDetachment(det.id)}
                          >
                            EJECT
                          </button>
                        )}
                  ...
                  {(!campaign?.participatingDetachments ||
                    campaign.participatingDetachments.length === 0) && (
                    <div className="restricted-text">
                      NO DETACHMENTS CURRENTLY DEPLOYED IN THIS THEATER.
                    </div>
                  )}
                </div>
              </div>
      ...
      {showMonthlyExpensesEditor !== null &&
        campaign &&
