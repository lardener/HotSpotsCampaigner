...
export const ActiveCampaignsList: React.FC<ActiveCampaignsListProps> = ({ onSelectCampaign }) => {
  ...
  const filteredCampaigns = useMemo(() => {
    ...
    return campaigns.filter((c): c is NonNullable<typeof c> => {
      ...
      return terms.some((term) => {
        ...
        const inDetachments = c.participatingDetachments?.some(
          (det) =>
            det != null &&
  ...
  return (
    <section className="active-campaigns-container" style={{ position: 'relative' }}>
      ...
      <div
        className="mb-20 mt-20 recruitment-box border-dashed-gray status-bar"
        style={{
          padding: '0 10px',
          display: 'flex',
          alignItems: 'center',
      ...
      >
        <span className="restricted-text sm-text mr-10" style={{ whiteSpace: 'nowrap' }}>
          SEARCH INTEL:
        </span>
        <input
          type="text"
      ...
      <div className="campaign-list">
        {filteredCampaigns.length === 0 ? (
          <p>
            {searchTerm
              ? 'No intel matches current search parameters.'
              : 'No active deployments reported in this sector.'}
          </p>
        ) : (
          filteredCampaigns
            ...
            .map((c) => (
              ...
              >
                ...
                <div className="flex flex-gap-20 fs-09em accent-dim-text">
                  <span>
                    <strong>Primary:</strong> {c.primaryEmployer}
                  </span>
                  <span>
                    <strong>Opposition:</strong> {c.secondaryEmployer}
                  </span>
                  <span>
                    <strong>Tracks:</strong> {c.trackCount}
                  </span>
                </div>
                {c.participatingDetachments && c.participatingDetachments.length > 0 && (
                  <div
                    className="mt-10"
                    style={{ borderTop: '1px dashed var(--accent-dim)', paddingTop: '10px' }}
                  >
                    <div className="restricted-text xs-text mb-5">DEPLOYED FORCES</div>
                    <div className="flex flex-wrap flex-gap-10">
                      {c.participatingDetachments
                        .filter((d) => d != null)
                        .map((d) => (
                          <span
                            key={d.id}
