...
export const CommandDashboard: React.FC<CommandDashboardProps> = ({
  ...
  const unitStatuses = (data?.publicCampaignMetadata?.unitStatuses || FALLBACK_STATUSES).filter(
    (x): x is UnitStatus => x !== null,
  )
  const unitTypes = (data?.publicCampaignMetadata?.unitTypes || FALLBACK_TYPES).filter(
    (x): x is UnitType => x !== null,
  )
  const techBases = (data?.publicCampaignMetadata?.techBases || FALLBACK_TECH).filter(
    (x): x is TechBase => x !== null,
  )
  ...
  const [assignAsset] = useMutation(ASSIGN_ASSET, {
    update(cache: ApolloCache, { data: result }, { variables }) {
      if ((result as any)?.assignAsset && variables) {
        ...
        const existing = cache.readQuery<UnitDossierData>({
        ...
        if (existing?.getCommand) {
          ...
          if (assetType === 'UNIT') {
            getCommand.units = (getCommand.units || [])
              .filter((u): u is NonNullable<typeof u> => u != null)
              .map((u: CombatUnit) =>
                u.id === assetId ? { ...u, detachmentId: detachmentId ?? null } : u,
              )
          } else {
            getCommand.pilots = (getCommand.pilots || [])
              .filter((p): p is NonNullable<typeof p> => p != null)
              .map((p: Pilot) =>
                p.id === assetId ? { ...p, detachmentId: detachmentId ?? null } : p,
              )
          }
          cache.writeQuery({
            query: GET_UNIT_DOSSIER,
            variables: queryVars,
            data: { ...existing, getCommand },
          })
        }
      }
    },
  ...
  const [deleteDetachment] = useMutation(DELETE_DETACHMENT, {
    update(cache: ApolloCache, { data: result }, { variables }) {
      if ((result as any)?.deleteDetachment && variables?.detachmentId) {
        cache.evict({
          id: cache.identify({ __typename: 'Detachment', id: variables.detachmentId }),
        })
        cache.gc()
      }
    },
        ...
        cache.writeQuery({
          ...
          data: {
            ...
            getCommand: {
              ...existing.getCommand,
              detachments: [
                ...(existing.getCommand.detachments || []),
                (createData as any).createDetachment,
              ],
            },
          },
        })
  ...
  const handleLeaveCampaign = async () => {
    ...
    setOverlay({
      ...
      onConfirm: async () => {
        ...
        setIsSyncing(true)
        try {
          await assignDetachment({
            variables: { detachmentId: selectedDetachmentId, campaignId: null },
          })
          refetch()
          onRefreshTree?.()
        } catch (err) {
  ...
  if (loading && !data)
    return <div className="loading-intel pulse"> RETRIEVING UNIT DOSSIER...</div>
  ...
  const detachments: Detachment[] = (command.detachments || []).filter(
    (d): d is any => d != null,
  ) as Detachment[]
  ...
  return (
    ...
    >
      ...
      <header
        className="dashboard-header dossier-header"
        style={{
        ...
        <div>
          {isEditingName ? (
            <div className="flex flex-gap-10 items-center">
              ...
              <h1 className="terminal-text" style={{ margin: 0 }}>
                {' '}
                - COMMAND DASHBOARD
              </h1>
            </div>
            ...
            >
              {command.name} - COMMAND DASHBOARD
            </h1>
          )}
        </div>
        <div className="sync-indicator text-right pb-5">
          <span
            className={`restricted-text ${isSyncing ? 'pulse' : ''}`}
            style={{
              color: isSyncing ? 'var(--terminal-amber)' : 'var(--terminal-green)',
              fontSize: '0.7rem',
            }}
          >
      ...
      <div className="dashboard-grid" style={{ gridTemplateColumns: '1fr' }}>
          ...
          <section className="dashboard-section tactical-panel" data-id="FORCE-SUMMARY">
            <div style={{ display: 'flex', gap: '25px' }}>
              <div
                style={{
                  width: '220px',
                  borderRight: '1px dashed var(--accent-dim)',
                  paddingRight: '25px',
                }}
                data-id="ORG-STRUCTURE"
              >
                ...
                {!isManagerView && (
                  <div style={{ display: 'flex', gap: '5px', marginBottom: '10px' }}>
                    ...
                    <button className="mode-btn" onClick={handleCreateDetachment}>
                      +
                    </button>
                  </div>
                )}
                    ...
                    <div key={det.id} style={{ display: 'flex', gap: '5px', marginBottom: '10px' }}>
                      ...
                      {!isManagerView && (
                        <button
                          ...
                          onClick={() => handleDeleteDetachment(det.id)}
                        >
                          X
                        </button>
                      )}
                ...
                {selectedDetachmentId &&
                  (() => {
                    const filteredDetachments = (command.detachments || []).filter(
                      (d): d is Detachment => d != null,
                    )
                    const currentDet = filteredDetachments.find(
                      (d: Detachment) => d.id === selectedDetachmentId,
                    )
                      ...
                      return (
                        ...
                        >
                          <h4
                            ...
                            style={{ fontSize: '0.6rem', marginBottom: '10px' }}
                          >
                            ACTIVE DEPLOYMENT
                          </h4>
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          className="mode-btn"
                          style={{ width: '100%', marginTop: '5px', fontSize: '0.7rem' }}
              ...
              <div style={{ flex: 1 }}>
                <h3 className="zone-header">
                  {selectedDetachmentId ? 'DETACHMENT READINESS' : 'COMMAND READINESS'}
                </h3>
        ...
        {!isManagerView && (
          <section
            className="dashboard-section tactical-panel"
            data-id={selectedDetachmentId ? 'DETACHMENT-LEDGER' : 'COMMAND-LEDGER'}
            style={{ gridColumn: '1 / -1', marginTop: '30px' }}
          >
            <h3 className="zone-header">
              {selectedDetachmentId ? 'DETACHMENT LEDGER' : 'COMMAND LEDGER'}
            </h3>
            ...
            >
              <LedgerEntryForm
                ...
                campaignId={
                  command.detachments
                    ?.filter((d: any): d is Detachment => d != null)
                    .find((d: Detachment) => d.id === selectedDetachmentId)?.campaignId
                }
                initialCampaignName={
                  command.detachments
                    ?.filter((d: any): d is Detachment => d != null)
                    .find((d: Detachment) => d.id === selectedDetachmentId)?.campaignName || ''
                }
            ...
            {sortedLedger.length > entriesPerPage && (
              <div
                className="flex-between items-center mt-15 pt-10"
                style={{ borderTop: '1px dashed var(--accent-dim)' }}
              >
