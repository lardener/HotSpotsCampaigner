...
export const calculatePilotFinancials = (
  pState: { healed: number },
  terms: { support: SupportTerms },
  healCost: number,
  pricingRule: 'Core' | 'Alpha Strike' = 'Core',
) => {
  // Alpha Strike: only 1 wound max can be healed
  const effectiveHealCount =
    pricingRule === 'Alpha Strike' ? Math.min(pState.healed, 1) : pState.healed
...
export const calculateAwardFinancials = (
  campaign: CampaignDetail,
  terms: DetachmentAarState & { payRate: number; salvageCoverage: number },
) => {
  ...
  const payAward = Math.round(
    baseCombatPay * (terms.outcomeMultiplier ?? 1) * (terms.selectedLevel ?? 1),
  )
...
export const calculateUnitFinancials = (
  unit: CombatUnit,
  status: string,
  rules: Partial<CampaignDetail> | undefined,
  statuses: UnitStatus[],
  pricingRule: 'Core' | 'Alpha Strike' = 'Core',
) => {
