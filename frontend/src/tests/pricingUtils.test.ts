...
import { getTechTax, calculatePurchasePrice, calculateReconfigureCost } from '../util/pricingUtils'
...
describe('getTechTax', () => {
  ...
  it('uses metadata defaults when not provided', () => {
    expect(getTechTax('Clan', { clanTechModifier: 3.0 } as PublicCampaignMetadataWithRules)).toBe(
      3.0,
    )
    expect(getTechTax('Mixed', { mixedTechModifier: 2.5 } as PublicCampaignMetadataWithRules)).toBe(
      2.5,
    )
  })
})
