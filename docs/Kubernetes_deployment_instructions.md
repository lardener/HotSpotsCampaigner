...
1. **Namespace** (`battletech-campaigner`): All resources are deployed into this dedicated namespace.
...
1. **Backend** (`ovhcloud-backend-deployment.yaml`):
...
1. **Prepare the Environment:** Ensure your `ovhcloud-1.0.0.env` (or similar) contains the correct image digests and credentials.
1. **SSL/TLS Certificate Generation:**
   ...
   ```
1. **Ingress Controller:** Ensure the Nginx Ingress Controller is installed:
   ```bash
...
2. **Deploy Secrets and ConfigMaps:**
   ...
   kubectl apply -f ovhcloud-auth0-secret.yaml
